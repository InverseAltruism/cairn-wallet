...
Version in tree: 0.2.55 (package.json / manifest / inpage in lockstep). Latest published GitHub release: v0.2.54; the Chrome Web Store version may lag a release or two behind it (store review plus the manual upload step). The 0.2.55 batch: honest structured send errors (incl. SUBMIT_MAYBE_INFLIGHT / SUBMIT_DUPLICATE with locally computed txids), coin consolidation + preview + the pending-merge indicator and merge-aware refusal copy, bounded verify concurrency, timeout-inversion fixes (GET 12s, trade/clarvis 12s, headers 15s + Retry-After), unlimitedStorage for the growing SPV snapshot, clarvis resolve fallback, 7-day pending-content retention. Vendored core: cairnx-core 0.1.35 at the csd-sdk commit pinned in src/vendor/PROVENANCE.json.
