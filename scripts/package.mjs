// Deterministic packager: zips dist/ into cairn-wallet.zip with a fixed layout,
// fixed timestamps, and STORE (no compression) so the output is BYTE-IDENTICAL on
// every machine for the same source. That makes the published SHA-256 reproducible:
// anyone can `npm ci && npm run package` and confirm the hash matches the release.
// Dependency-free (no `zip` binary, no archiver lib) — minimal ZIP writer below.
...
const PREFIX = "cairn-wallet"; // unzips to a cairn-wallet/ folder (Load unpacked → pick it)
...
const files = walk(DIST).map((abs) => ({
  name: PREFIX + "/" + relative(DIST, abs).split("\\").join("/"),
...
writeFileSync(join(ROOT, "cairn-wallet.zip"), zip);
writeFileSync(join(ROOT, "cairn-wallet.zip.sha256"), sha + "  cairn-wallet.zip\n");
console.log(`✓ cairn-wallet.zip (${files.length} files, ${zip.length} bytes)`);
