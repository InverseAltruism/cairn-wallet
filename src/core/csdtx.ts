...
function serializeApp(app: App): Uint8Array {
  ...
  if (app.type === "Propose") return concatBytes(u32(1), lenBytes(utf8ToBytes(app.domain)), hb(app.payloadHash), lenBytes(utf8ToBytes(app.uri)), u64(app.expiresEpoch));
  return concatBytes(u32(2), hb(app.proposalId), u32(app.score), u32(app.confidence));
}

export function serialize(tx: Tx): Uint8Array {
  const parts: Uint8Array[] = [u32(tx.version), u64(tx.inputs.length)];
  for (const i of tx.inputs) parts.push(hb(i.prevTxid), u32(i.vout), lenBytes(hb(i.scriptSig)));
  parts.push(u64(tx.outputs.length));
  for (const o of tx.outputs) parts.push(u64(o.value), hb(o.scriptPubkey));
